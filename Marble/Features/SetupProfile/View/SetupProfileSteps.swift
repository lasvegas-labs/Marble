...
struct TimeLostCalculationStepView: View {
    @ObservedObject var viewModel: SetupProfileViewModel
    ...
    var body: some View {
        ZStack {
            ...
            // Dots and numbers along the arc
            VStack {
                Spacer()
                ...
                HStack {
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text("Lost to Your Screen")
                                .font(.system(size: 11, weight: .semibold, design: .rounded))
                                .foregroundColor(.secondary)
                            
                            Text("See time cost of your current habits")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 16)
                            .fill(Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.12))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 16)
                            .stroke(Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.3), lineWidth: 1)
                    )
                    .shadow(color: Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.1), radius: 8, y: 4)
                    .offset(x: 60)
...
struct TimeSavedEffectStepView: View {
    @ObservedObject var viewModel: SetupProfileViewModel

    var body: some View {
        VStack(spacing: 32) {
            VStack(spacing: 8) {
                Text("Marble can save your time:")
            ...
            VStack(spacing: 16) {
                effectRow(title: "Knowledge", isHighlighted: false)
                effectRow(title: "Health", isHighlighted: false)
                effectRow(title: "Skill", isHighlighted: true)
                effectRow(title: "Experiences", isHighlighted: false)
            }
    ...
    private func effectRow(title: String, isHighlighted: Bool) -> some View {
        Text(title)
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    .fill(isHighlighted 
                          ...
                          : Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.12))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 16)
                    .stroke(isHighlighted 
                            ? Color.blue 
                            : Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.3), 
                            lineWidth: 1.5)
            )
            .shadow(color: isHighlighted 
                    ? Color.blue.opacity(0.15) 
                    : Color(red: 0.0, green: 0.82, blue: 0.58).opacity(0.1), 
                    radius: 8, y: 4)
    }
}
...
struct NotificationPermissionStepView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            StepHeader(
                title: "Stay Motivated with Our Notifications",
                subtitle: "Enable notifications so we can gently remind you when it's time to take a break and help you stay on track with your screen time goals."
            )
            ...
            ZStack(alignment: .bottomLeading) {
                ...
                RoundedRectangle(cornerRadius: 32)
                    ...
                    .overlay(
                        // Inner Mock System Permission Card (Light Glassmorphism with Margin)
                        VStack(spacing: 12) {
                            Text("\"Marble\" Would Like to Send You Notifications")
                                .font(.system(size: 17, weight: .bold, design: .rounded))
                                .foregroundColor(.primary)
                                .multilineTextAlignment(.center)
                ...
                // Guide arrow pointing to Allow button area
                VStack(spacing: 4) {
                    CurvedArrow()
                    Text("Tap Allow")
                        .font(.system(size: 12, weight: .bold, design: .rounded))
